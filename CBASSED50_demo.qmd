...
exploratory_curve <-
  ggplot(data = cbass_dataset,
       aes(
         x = Temperature,
         y = PAM,
         # You can play aroung the group value (e.g., Species, Site, Condition)
...
summary_ed50_df <- ed50_df %>%
  ...
  summarise(Mean_ED50 = mean(ED50),
            ...
            Conf_Int_lower = Mean_ED50 - qt(0.975, df = n() - 1) * SE_ED50,
            Conf_Int_upper = Mean_ED50 + qt(0.975, df = n() - 1) * SE_ED50) %>%
  mutate(across(c(Mean_ED50,
                  SD_ED50, SE_ED50,
...
result_df <- predictions_df %>%
  left_join(summary_ed50_df, by = c("Site", "Condition", "Species", "Timepoint"))
```
